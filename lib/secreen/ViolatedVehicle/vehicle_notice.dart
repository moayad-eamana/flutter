...
class vehicle_notice extends StatefulWidget {
  const vehicle_notice({Key? key}) : super(key: key);

  @override
  State<vehicle_notice> createState() => _vehicle_noticeState();
}

class _vehicle_noticeState extends State<vehicle_notice> {
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBarW.appBarW("إشعار", context, null),
                            ...
                            widgetsUni.actionbutton('إستعلام', Icons.send,
                              ...
                              if (groupValue == 2) {
                                ...
                                if (jsonDecode(respons.body)["StatusCode"] ==
                                    400) {
                                  violatedVehicle["carInfo"] =
                                      jsonDecode(respons.body)["ResponseData"]
                                          ["data"]["result"];
                                  print(violatedVehicle["carInfo"]);
                                  EasyLoading.dismiss();
                ...
                Column(
                  children: [
                    Row(
                      children: [
                        cards("هوية المالك",
                            violatedVehicle["PersonId"].toString()),
                      ],
                    ),
                    SizedBox(
                      height: 5,
                    ...
                    Row(
                      children: [
                        cards("السيارة", violatedVehicle["MakeDescAr"]),
                        cards("نوع السيارة", violatedVehicle["ModelDescAr"])
                      ],
                    ),
                    Row(
                      children: [
                        cards("موديل السيارة",
                            violatedVehicle["ModelYear"].toString()),
                        cards(
                            "الحالة",
                            violatedVehicle["IsAlive"] == 1
                                ? "على قيد الحياة"
                                : "متوفى"),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        cards(
                            "لون السيارة", violatedVehicle["MajorColorDescAr"])
                      ],
                    ),
                    SizedBox(
                      height: 5,
                    ...
                    Container(
                        width: 150,
                        child: widgetsUni.actionbutton("إشعار", Icons.send,
                            () async {
                          Alerts.confirmAlrt(
                                  context, "", "!هل أانت متأكد", "نعم")
                              .show()
                              .then((value) async {
                            if (value == true) {
                              EasyLoading.show(
                              ...
                              var respons = await postAction(
                                  "ViolatedCars/SendNotification",
                                  jsonEncode({
                                    "EmployeeNumber":
                                        EmployeeProfile.getEmployeeNumber(),
                                    "IdNumber": violatedVehicle["PersonId"],
                                    "PlateNumber": _platenumber.text,
                                    "PlateText": _plateText.text
                                  }));
